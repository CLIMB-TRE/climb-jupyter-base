...
After editing Python files (like `climb_jupyter_base/routes.py`):

```bash
python -m py_compile climb_jupyter_base/__init__.py  # Check single file for syntax errors
```
...
**Python** (in `climb_jupyter_base/*.py` files):
...
- Backend processing in `climb_jupyter_base/routes.py`
...
- **✅ Do**: `climb_jupyter_base/` with underscores, all lowercase
...
```typescript
const PLUGIN_ID = 'climb_jupyter_base:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'climb_jupyter_base:open-panel';
  export const refreshData = 'climb_jupyter_base:refresh-data';
}
...
- Backend routes: `climb_jupyter_base/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `climb_jupyter_base/` directory
...
1. **Read the backend first** — Check `climb_jupyter_base/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `climb_jupyter_base/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "climb_jupyter_base", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `climb_jupyter_base/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `climb_jupyter_base/`)
...
5. **Install the extension** (REQUIRED for JupyterLab to recognize it):
   ...
   jupyter server extension enable climb_jupyter_base
   ```
...
- **Output**: Creates files in `lib/` and `climb_jupyter_base/labextension/`
...
**After editing Python** (files in `climb_jupyter_base/`):
...
- Ask user to search for the extension ID (`climb_jupyter_base`) to see if it loaded
...
ls -la climb_jupyter_base/labextension/  # Should contain bundled extension
```
...
- Backend routes: `climb_jupyter_base/routes.py`
- Server extension config: `jupyter-config/server-config/climb_jupyter_base.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `climb_jupyter_base/routes.py`
...
**❌ Don't**: Use absolute imports like `from climb_jupyter_base.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'climb_jupyter_base:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'climb_jupyter_base:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-climb-jupyter-base-ClassName`
- **API routes** (in `climb_jupyter_base/routes.py`): `url_path_join(base_url, "climb_jupyter_base", "endpoint")`
